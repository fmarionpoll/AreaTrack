...
//	private int thresholdForHeatMap = 230;
	...
	public void initAreaDetectionFromColors(SequenceVirtual sequenceVirtual, ArrayList<ROI2D> roiList,
	...
	public void initMovementDetection(SequenceVirtual sequenceVirtual, ArrayList<ROI2D> roiList,
...
//						IcyBufferedImage sourceImage = vSequence.imageIORead(vSequence.getFileName(iframe));
	...
	void updateDisplay(int t, int nbframes, Chronometer chrono, ProgressFrame progress) {
		...
		progress.setMessage("Processing: " + pos + " % - Elapsed time: " + nbSeconds + " s - Estimated time left: "
				+ timeleft + " s");
	}

	private void detectMovements(ArrayList<BooleanMask2D> areaMasks) throws InterruptedException {
		// update resultIlmage to make it like a regular image and add heatmap scale to
		// it
		...
		for (int c = 0; c < cmax; c++) {
			double[] resultDoubleArray = Array1DUtil.arrayToDoubleArray(resultONImage.getDataXY(c),
					resultONImage.isSignedDataType());
		...
		// ------------------------ loop over all the cages of the stack & count n
